...
# NonBlocking I/O: 호출된 함수가(서브루틴) return 후 호풀한 함수(메인 루틴)에 제어권 전달 -> 타 함수는 일 지속
...
async def main():
    ...
    futures = [
        asyncio.ensure_future(fetch(url, executor)) for url in urls     # url 하나당 하나의 쓰레드
    ]
    
    # 결과 취합
    rst = await asyncio.gather(*futures)    # list니까 unpacking
