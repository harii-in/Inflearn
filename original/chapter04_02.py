...
# tuple은 불변형이므로 수정을 할 수 없으므로 id값이 계속 바뀜.
# list는 가변형이므로 자기 원래 id값에 할당.
...
print('sorted -', sorted(f_list, key=len))      # 알파벳의 길이 순
print('sorted -', sorted(f_list, key=lambda x: x[-1]))      # 마지막 알파벳을 기준으로 정렬
